#!/usr/bin/bash
#SBATCH -p batch -c 2 --mem 8gb --time=3-00:00:00 --out logs/pezizo5_mmseqs_refresh.log

module load nextflow

# Fresh (non -resume) re-run of the pezizo5_mmseqs family-profile (mmseqs) novelty/loss pipeline, to pick up
# the presence-calling fixes merged in PR #59 (H2 paralog-cutoff fix a49ee0c,
# the coverage-double-counting fix in bin/profile_to_matrix.py, 2026-09-03). The original results/pezizo5_mmseqs run predates that
...
#
# --project pezizo5_mmseqs is kept so this reuses results/pezizo5_mmseqs/search_cache/
...
#
# Isolated launch dir (.nf_launch/pezizo5_mmseqs_refresh/) so this session/work/ dir
...
CONFIG=$(abspath configs/pezizo5.csv)
...
LAUNCH_DIR="$REPO_ROOT/.nf_launch/pezizo5_mmseqs_refresh"
mkdir -p "$LAUNCH_DIR"
cd "$LAUNCH_DIR"
