#!/usr/bin/bash
#SBATCH -p batch -c 2 --mem 8gb --time=3-00:00:00 --out logs/Chaeto_subset_refresh.log

module load nextflow

# Fresh (non -resume) re-run of the Chaeto_subset family-profile (mmseqs) novelty/loss pipeline, to pick up
# the presence-calling fixes merged in PR #59 (H2 paralog-cutoff fix a49ee0c,
# the coverage-double-counting fix in bin/profile_to_matrix.py, 2026-09-03). The original results/Chaeto_subset run predates that
# fix. Not using -resume: modules/profile_presence_matrix.nf's process script was NOT
...
#
# --project Chaeto_subset is kept so this reuses results/Chaeto_subset/search_cache/
...
#
# Isolated launch dir (.nf_launch/Chaeto_subset_refresh/) so this session/work/ dir
...
CONFIG=$(abspath configs/Chaetothyriales_subset.csv)
...
LAUNCH_DIR="$REPO_ROOT/.nf_launch/Chaeto_subset_refresh"
mkdir -p "$LAUNCH_DIR"
cd "$LAUNCH_DIR"
