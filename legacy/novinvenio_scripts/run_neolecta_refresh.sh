#!/usr/bin/bash
#SBATCH -p batch -c 2 --mem 8gb --time=2-00:00:00 --out logs/neolecta_refresh.log

module load nextflow

# Fresh (non -resume) re-run of the neolecta pairwise novelty/loss pipeline, to pick up
# the presence-calling fixes merged in PR #59 (H2 paralog-cutoff fix a49ee0c,
# 2026-09-03). The original results/neolecta run predates that
...
#
# --project neolecta is kept so this reuses results/neolecta/search_cache/
...
#
# Isolated launch dir (.nf_launch/neolecta_refresh/) so this session/work/ dir
...
CONFIG=$(abspath configs/nirr.csv)
...
LAUNCH_DIR="$REPO_ROOT/.nf_launch/neolecta_refresh"
mkdir -p "$LAUNCH_DIR"
cd "$LAUNCH_DIR"
