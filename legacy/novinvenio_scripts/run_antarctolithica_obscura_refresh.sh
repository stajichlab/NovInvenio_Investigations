#!/usr/bin/bash
#SBATCH -p batch -c 2 --mem 8gb --time=2-00:00:00 --out logs/Antarctolithica_obscura_refresh.log

module load nextflow

# Fresh (non -resume) re-run of the Antarctolithica_obscura pairwise novelty/loss pipeline, to pick up
# the presence-calling fixes merged in PR #59 (H2 paralog-cutoff fix a49ee0c,
# 2026-09-03). The original results/Antarctolithica_obscura run predates that
...
#
# --project Antarctolithica_obscura is kept so this reuses results/Antarctolithica_obscura/search_cache/
...
#
# Isolated launch dir (.nf_launch/Antarctolithica_obscura_refresh/) so this session/work/ dir
...
CONFIG=$(abspath configs/Antarctolithica_obscura.csv)
...
LAUNCH_DIR="$REPO_ROOT/.nf_launch/Antarctolithica_obscura_refresh"
mkdir -p "$LAUNCH_DIR"
cd "$LAUNCH_DIR"
...
nextflow run "$REPO_ROOT/main.nf" \
    ...
    --outdir "$OUTDIR" \
    --project Antarctolithica_obscura \
    -profile slurm \
    -c "$SITE_CONFIG"
