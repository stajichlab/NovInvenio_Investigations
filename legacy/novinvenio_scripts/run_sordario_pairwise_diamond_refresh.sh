#!/usr/bin/bash
#SBATCH -p batch -c 2 --mem 8gb --time=2-00:00:00 --out logs/sordario_pairwise_diamond_refresh.log

module load nextflow

# Fresh (non -resume) re-run of the sordario_pairwise_diamond pairwise novelty/loss pipeline, to pick up
# the presence-calling fixes merged in PR #59 (H2 paralog-cutoff fix a49ee0c,
# 2026-09-03). The original results/sordario_pairwise_diamond run predates that
...
#
# --project sordario_pairwise_diamond is kept so this reuses results/sordario_pairwise_diamond/search_cache/
# (storeDir is keyed by outdir/project/search_cache), so the expensive
# diamond pairwise + self searches are NOT re-run -- only
# BUILD_PRESENCE_MATRIX and everything downstream recomputes.
#
# Isolated launch dir (.nf_launch/sordario_pairwise_diamond_refresh/) so this session/work/ dir
...
CONFIG=$(abspath configs/sordario_pairwise.csv)
...
LAUNCH_DIR="$REPO_ROOT/.nf_launch/sordario_pairwise_diamond_refresh"
mkdir -p "$LAUNCH_DIR"
cd "$LAUNCH_DIR"
...
nextflow run "$REPO_ROOT/main.nf" \
    ...
    --outdir "$OUTDIR" \
    --project sordario_pairwise_diamond \
    -profile slurm \
    -c "$SITE_CONFIG"
