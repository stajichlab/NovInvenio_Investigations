...
| tier | wall_hours | cpu_hours | note |
|---|---|---|---|
| P | 0.192 | 2.721 | COMPLETE: all-vs-all search measured from 36 *_SEARCH/*_MAKEDB trace rows (0.091 wall-h, 2.244 cpu-h), plus DIAMOND_SELF, PARSE_HITS, PARSE_SELF_HITS, TBLASTN(+MAKEDB), BUILD_PRESENCE_MATRIX. |
| C+H | 31.287 | 641.182 | PROFILE_SEARCH:* (gain-side family-profile pathway), measured directly. |
| C+H_loss | 96.206 | 2663.815 | PROFILE_LOSS_SEARCH:* (loss-side equivalent), measured directly. |
| C | 0.0 | 0.0 | Free byproduct of Tier C+H's own clustering step. |
| R | 0.011 |  | Measured directly with `time` around refine_ambiguous_families.py (gain) -- not read from any trace file (plain subprocess call, no trace row). cpu_hours not measured. |
| R_loss | 0.102 |  | Measured directly with `time` around refine_ambiguous_families.py (loss) -- not read from any trace file (plain subprocess call, no trace row). cpu_hours not measured. |

### Analysis 3 notes (hand-written, 2026-09-23)

The table above is `bin/trace_cost_report.py` output. The first version of this report used the old tool, which labelled Tier P "unmeasurable" and left out the loss-side Tier R. The tool is now fixed, and the table above is regenerated. The details:

- **Tier P is not a lower bound here.** The old tool hard-coded the "storeDir cache hit, unmeasurable" note. For sordariales_shallow the all-vs-all diamond search ran fresh in trace `sordariales_shallow-20260922_101317`, so its trace rows exist. Measured from that trace (realtime, completed tasks):
...
3. **Genome-wide concordance (Analysis 2).** C+H against P: gain precision 0.53, recall 0.19 (208 vs 568 candidates); loss precision 0.46, recall 0.21 (749 vs 1620). C+H misses about 80% of the P candidates in both directions. Per-miss classification (`bin/classify_concordance_misses.py`, added 2026-09-23): gain misses are 231 clustering-level and 227 HMM-level; loss misses are 291 clustering-level and 986 HMM-level. See `notes/cluster-vs-pairwise/README.md` for all three clades.
