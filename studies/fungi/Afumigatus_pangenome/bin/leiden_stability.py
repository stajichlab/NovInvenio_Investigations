#!/usr/bin/env python3
"""Empirically test whether the Leiden trans-co-occurrence module
resolution has a defensible value, per PANGENOME_CLUSTER_PROFILE_NOTES.md's
2026-09-15 "module count and singleton fraction both grow steeply with
resolution... no obviously right resolution value" finding.

Seed-stability check: at each candidate resolution, rerun Leiden with many
different random seeds and measure how much the resulting partition agrees
across seeds via Adjusted Mutual Information (AMI). A resolution where
independent seeds converge on nearly the same partition (AMI near 1)
reflects real, reproducible community structure; a resolution where AMI is
low means the partition is close to arbitrary tie-breaking among
near-degenerate solutions, not a stable answer.
...
def load_cached_trans_edges(path: str) -> list[tuple[str, str, float]]:
    """Reads a pre-extracted trans-edge cache (family_a, family_b, fdr_q),
    e.g. produced once via:
      awk -F'\\t' '$3=="trans"{print $1"\\t"$2"\\t"$7}' pair_classification.rescued.tsv
    -- avoids re-scanning the multi-GB pair_classification file for every
    seed/resolution combination in the sweep."""
    edges = []
...
def pairwise_ami(partitions: list[dict], families: list[str]) -> tuple[float, float]:
    """Mean and stdev of Adjusted Mutual Information over all pairs of
    partitions (e.g. one per random seed at a fixed resolution)."""
    scores = []
