...
def test_two_tier_families_attaches_superfamily_label(tmp_path):
    ...
    # three tier-1 families: repA (with member memberA2), repB (singleton), repC (singleton)
    ...
    # but repC is not present in tier-2, so it defaults to itself
    ...
    # repC is not in tier-2, so superfamily defaults to itself
    assert result["repC"]["members"] == ["repC"]
    assert result["repC"]["superfamily"] == "repC"
