#!/usr/bin/env python3
"""One-off pull for the Akkermansia muciniphila ATCC BAA-835 model organism, used
by studies/bacteria/UHM_Akkermansia/modelorgs.yaml (AkkMuc entry).

ATCC BAA-835 is the type strain of the genus Akkermansia -- the reference the
user asked to set up a protein lookup for, same pattern as UHM_Koxytoca's
Kpn78578 model organism (see studies/bacteria/UHM_Koxytoca/bin/fetch_kpn78578_modelorg.py,
whose structure this mirrors almost exactly).

Fetches both sources agreed on for this model organism:
  1. NCBI RefSeq assembly GCF_000020225.1 (genome + GFF3 + protein.faa, single
     replicon: chromosome NC_010655.1 / GenBank CP001071) via
  ...
  2. UniProt reference proteome UP000001031 (taxid 349741) -- confirmed via
     https://rest.uniprot.org/proteomes/search?query=taxonomy_id:349741 to be the
     exact ATCC BAA-835 strain proteome, whose sole component cross-references
     GenBank accession CP001071 -- the same chromosome NCBI's GCF_000020225.1
     wraps, confirming the two sources agree.

Both land in the ephemeral, gitignored data/ caches with provenance sidecars (never
committed raw -- CLAUDE.md's provenance rule). NCBI's GCF_000020225.1 pull
(genome+GFF3+protein) is kept as reference data only -- it is NOT what
modelorgs.yaml's diamond_fasta lookup uses, because its protein_fasta and
gene_names_csv would then live in two different ID spaces (NCBI WP_/locus-tag
accessions vs. UniProt accessions from bin/extract_dat_annotations.py's own
.dat-derived TSV). Instead this script builds the diamond_fasta lookup entirely
from the UniProt UP000001031 pull, single ID space throughout:
  - config_support/modelorgs/AkkMuc_protein.faa -- the UniProt fasta with headers
  ...
  - config_support/modelorgs/AkkMuc_gene_names_UniProt.tsv (UniProt DR-derived
    gene_name/description/GO/Pfam, via bin/extract_dat_annotations.py, keyed by
    the same UniProt accession -- modelorgs.yaml's gene_names_csv)
...
NCBI_ACCESSION = "GCF_000020225.1"
UNIPROT_PROTEOME_ID = "UP000001031"
UNIPROT_TAXID = "349741"
...
def check_datasets_on_path() -> str:
    ...
    if not exe:
        sys.exit(
            ...
            "Run: source /etc/profile.d/modules.sh && module load ncbi_datasets"
        )
    return exe
...
def fetch_ncbi(outdir: Path) -> Path:
    ...
    print(f"[AkkMuc] datasets download genome accession {NCBI_ACCESSION} --include genome,gff3,protein", file=sys.stderr)
    subprocess.run(
    ...
    for f, kind in (
        ...
        print(f"[AkkMuc] OK  {kind}: {f.name}  sha256={checksum[:12]}...", file=sys.stderr)
...
def fetch_uniprot(outdir: Path) -> Path:
    subprocess.run(
        [sys.executable, str(NII_BIN / "fetch_uniprot_proteome.py"),
         ...
         "--outdir", str(outdir), "--short", "AkkMuc"],
        check=True,
    )
    ...
    with gzip.open(uniprot_fasta_gz, "rt") as fin, open(out_path, "w") as fout:
        for line in fin:
            if line.startswith(">"):
                ...
                fout.write(line)
    print(f"[AkkMuc] wrote {out_path} ({n} proteins, headers rewritten for diamond_fasta)", file=sys.stderr)
...
def main() -> int:
    ...
    tracked_protein = tracked_dir / "AkkMuc_protein.faa"
    write_diamond_ready_fasta(uniprot_fasta_gz, tracked_protein)

    gene_names_tsv = tracked_dir / "AkkMuc_gene_names_UniProt.tsv"
    subprocess.run(
